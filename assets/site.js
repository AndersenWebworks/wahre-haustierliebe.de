...
function normalizeAssetUrls(root) {
  Array.from((root || document).querySelectorAll('img[src^="assets/"]')).forEach(function(img) {
    ...
    function submitContactForm(form, submit, status) {
      ...
      var endpoint = form.getAttribute('action') || '/api/kontakt';
      var previousLabel = submit.textContent;
      submit.disabled = true;
      submit.textContent = 'Wird gesendet ...';
      setContactFormStatus(status, 'Wir senden deine Nachricht gerade.', '');
      ...
      fetch(endpoint, {
        ...
        body: JSON.stringify(contactFormPayload(form))
      })
        ...
        .then(function() {
          form.reset();
          setContactFormStatus(status, 'Danke. Deine Nachricht wurde gesendet.', 'success');
        })
        .catch(function() {
          setContactFormStatus(status, 'Das hat noch nicht geklappt. Bitte versuche es gleich noch einmal oder schreibe uns direkt per E-Mail.', 'error');
        })
        .finally(function() {
